...
def extract_features(model, data_loader, print_freq=100, metric=None):
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, _,_,_) in enumerate(data_loader):
            ...
            outputs_bn,outputs_af = extract_cnn_feature(model, imgs)
            for fname,output_bn,output_af, pid in zip(fnames, outputs_bn,outputs_af, pids):
                features_bn[fname] = output_bn
                features_af[fname] = output_af
                labels[fname] = pid
    ...
    return features_bn,features_af,labels
...
class Evaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery, metric=None, cmc_flag=False, rerank=False, pre_features=None):
        if (pre_features is None):
            _,features, _ = extract_features(self.model, data_loader)
        else:
