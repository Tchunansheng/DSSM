...
def _pluck_msmt(list_file, subdir, pattern=re.compile(r'([-\d]+)_([-\d]+)_([-\d]+)')):
    ...
    for line in lines:
        ...
        if pid not in pids:
            pids.append(pid)
        ret.append((osp.join(subdir,fname), pid, cam,torch.tensor(1.0)))
    return ret, pids
...
class Dataset_MSMT(object):
    ...
    def load(self, verbose=True):
        ...
        cam2uid = defaultdict(list)
        for i, (img_path, pid, camid, _) in enumerate(self.train):
            # print(camid)
            cam2uid[camid].append(i)
        self.cam2uid = cam2uid
